...
if ($privilege ==3) {
$query_coursecode = "
		SELECT DISTINCT submitresult_faculty.courseCode, submitresult_faculty.category,
						course.CourseName
		FROM submitresult_faculty 
			INNER JOIN course ON (course.CourseCode = submitresult_faculty.courseCode)
		WHERE (submitresult_faculty.acYear = '$ayear') 
		  ORDER BY submitresult_faculty.courseCode ASC";
...
?>
 <fieldset>
	<legend>Select Appropriate Entries</legend>
		<form action="dofassessedit.php" method="post" enctype="multipart/form-data" name="frmCourse" target="_self">
