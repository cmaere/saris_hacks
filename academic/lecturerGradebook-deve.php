...
function doquery()
{
 var coursecode = document.frmCourse.course.value
    ...
    self.location='lecturerGradebook-deve.php?coursecode=' + coursecode;
...
self.location='lecturerGradebook-deve.php?coursecode=' + course +'&cat=' + cat +'&examdate=' + examdate;
}
...
{
    self.location='lecturerGradebook-deve.php?catb=' + catid +'&catedisplay=' + catedisplay +'&coursecode=' + coursecode +'&examdate=' + examdate;
...
$query_userdept = "SELECT Dept,RegNo FROM security where UserName = '$username' AND Dept<>0";
$userdept = mysql_query($query_userdept, $zalongwa) or die(mysql_error());
$row_userdept = mysql_fetch_assoc($userdept);
$regno = $row_userdept["RegNo"];
...
// Process of exam grade form entry intro
    ...
    if ($privilege ==3) {
    $query_coursecode = "
            SELECT DISTINCT CourseCode, AYear
            FROM examregister 
                
            WHERE (AYear ='$ayear') 
            AND (RegNo='$username')  ORDER BY CourseCode ASC";
    ...
    ?>
     <fieldset>
        <legend>Select Appropriate Entries</legend>
            <form action="lecturerGradebookAdd-dev.php" method="post" enctype="multipart/form-data" name="frmCourse" target="_self">
