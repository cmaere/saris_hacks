 ...
 $this->Ln(7);
  $this->Ln(7);
   ...
   for($i=0;$i<count($header);$i++)
   {
	...
	{
    $sqli = "select distinct e.CourseCode, e.assessment_status  from examdate e, submitresult s where  e.CourseCode = s.courseCode and e.Ayear = $year and e.assessment_status = $cat  order by e.CourseCode asc ";
	}
	else
	{
		
	$sqli = "select distinct e.CourseCode, e.assessment_status  from examdate e, submitresult s where  e.CourseCode = s.courseCode and e.Ayear = $year  order by e.CourseCode asc ";	
...
require_once('../../../Connections/zalongwa.php');
 ...
 if($semister != "Semester I")
	{
	$sql2 = "select distinct er.RegNo from examregister er, examdate e where er.CourseCode = e.CourseCode  and er.Ayear = $year and e.assessment_status = $cat and er.RegNo like '%/%/%' order by er.RegNo asc ";
	}
	else
	{
		$sql2 = "select distinct er.RegNo from examregister er, examdate e where er.CourseCode = e.CourseCode  and er.Ayear = $year  and er.RegNo like '%/%/%' order by er.RegNo asc ";	
	}
  ...
  // die($sql2);
    while($row = mysql_fetch_array($result2, MYSQL_ASSOC))
    {
        ...
        $sql4b = "select UPPER(ex.RegNo) as RegNo, e.CourseCode, ex.ExamScore from examresult ex, examdate e where ex.CourseCode  = e.CourseCode  and ex.RegNo = '$regno' and ex.AYear = $year ";
...
if($semister != "Semester I")
	{
        $sql3 = "select distinct e.CourseCode, e.assessment_status  from examdate e, submitresult s where  e.CourseCode = s.courseCode and e.Ayear = $year and e.assessment_status = $cat  order by e.CourseCode asc";
        
	}
	else
	{
		$sql3 = "select distinct e.CourseCode, e.assessment_status  from examdate e, submitresult s where  e.CourseCode = s.courseCode and e.Ayear = $year   order by e.CourseCode asc";	
		...
		if($semister != "Semester I")
	{
        ...
        $sqlavg = "select AVG(ex.ExamScore) as avg from examresult ex, examdate e where ex.CourseCode  = e.CourseCode  and ex.RegNo = '$regno' and ex.AYear = $year  and ex.ExamCategory = $cat and e.assessment_status	 = $cat";
	}
	else
	{
		$sqlavg = "select AVG(ex.ExamScore) as avg from examresult ex, examdate e where ex.CourseCode  = e.CourseCode  and ex.RegNo = '$regno' and ex.AYear = $year  ";
