<?php
	...
	$sponsor = mysql_query($query_sponsor, $zalongwa) or die(mysql_error());
			...
			$row_result = mysql_fetch_assoc($dbRegNo);
				...
				if ($total>0) {
					...
					echo "<br> Go Back and Insert Newone!<hr><br>";
								...
								VALUES('$name','$regno','$sex','$dtDOB',' ','$maritalstatus',' ','$degree','$faculty',' ',' ','$gyear','$ayear','$status',' ','$address','$country','$region',
...
if (isset($_POST['actionupdate']) && ($_POST['actionupdate'] == "Update"))
	{
		...
		#check if RegNo Exist
			...
			if ($oldRegNo == '$regno') {
						...
						ProgrammeofStudy = '$degree', Subject = '$combi', Faculty = '$faculty',Department = ' ', Sponsor = '$sponsor', GradYear = '$gyear',EntryYear = '$ayear',Status = '$status',
				...
				$result = mysql_query($sqlUpdate) or die("Kunamatatizo ya SQL Syntax!" . mysql_error());
	...
	if (mysql_num_rows($query) > 0){
		...
		while($result = mysql_fetch_array($query)) {
			 ...
			 $dbdegree = mysql_query($qdegree, $zalongwa) or die(mysql_error());
...
-->
	  ...
	  <table width="465" border="1" cellpadding="0" cellspacing="0" bgcolor="#CCCCCC">
        ...
        <tr>
          ...
          <tr>
			  <td><div align="right">Degree Registered:<span class="style2">*</span></div></td>
			  <?php $degreecode = $row_update['ProgrammeofStudy'];
			  $qdegree = "SELECT ProgrammeName from programme WHERE ProgrammeCode = '$degreecode'";
			 $dbdegree = mysql_query($qdegree, $zalongwa) or die(mysql_error());
			  $row_dbdegree = mysql_fetch_assoc($dbdegree);
			  ?>
			  <td ><select name="degree" id="degree">
			  <option value="<?php echo $row_update['ProgrammeofStudy']?>"><?php echo $row_dbdegree['ProgrammeName']?></option>
						  <?php
								do {  
										?>
		  ...
		  <tr>
			  ...
			  <?php $subjectid = $row_update['Subject'];
...
<input name="dbirth" type="hidden" id="dbirth" value = "<?php echo $row_update['DBirth']?>"><?php echo $row_update['DBirth']?></font></div></TD>
		   ...
		   <option value="<?php echo $row_update['Religion']?>"><?php echo $row_update['Religion']?></option>
			...
			<tr>
			 <td nowrap><div align="right">Sect or Denomination: </div></td>
          	 <td><input name="denomination" type="text" id="denomination" value = "<?php echo $row_religion['Denomination']?>" size="30"></td>
		 </tr>
 		<tr>
          <td nowrap><div align="right"> Postal Address: </div></td>
           <td> <input name="postaladdress" type="text" id="postaladdress" size="30" value = "<?php echo $row_religion['Address']?>"></td>
		</tr>
		<tr>
			<td><div align="right">Residential Address: </div></td>
			<td><input name="residenceaddress" type="text" id="residenceaddress" size="30" value ="<?php echo $row_religion['Address']?>"></td>
		</tr>
		<tr>
		...
		<option value="<?php echo $row_update['Status']?>"><?php echo $row_update['Status']?></option>
                      <?php
							do {  
									?>
