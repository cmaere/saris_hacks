...
$sqli = "select distinct e.CourseCode, e.assessment_status  from examdate e, submitresult s where  e.CourseCode = s.courseCode and s.courseCode <> 'ED 502' and e.Ayear = $year and e.assessment_status = 5 and e.programme = '$program_previous' order by e.CourseCode asc ";
...
//Header
   ...
   for($i=0;$i<count($header);$i++)
   {
    ...
    $times = 1;
    $times2 = 1;
    $sqli = "select distinct e.CourseCode, e.assessment_status  from examdate e, submitresult s where  e.CourseCode = s.courseCode and e.Ayear = $year and s.courseCode <> 'ED 502' and e.assessment_status = 5 and e.programme = '$program_previous' order by e.CourseCode asc ";
        //die($sqli);
        $resulti = mysql_query($sqli);
...
//generation of courses for a specific programme
        $sql3 = "select distinct e.CourseCode, e.assessment_status  from examdate e, submitresult s where  e.CourseCode = s.courseCode and s.courseCode <> 'ED 502' and e.Ayear = $year and e.assessment_status = 5 and e.programme = '$program_previous' order by e.CourseCode asc ";
...
where AYear = $year and RegNo = '$regno' and ExamCategory = $cat  ";
        ...
        if($badseed !=1)
        {
            ...
            if($lowestmark_yr4_clinical > 69.4 && $lowestmark_yr4 >= 70 && $avg > 74.4 && $avg2 > 74.4 )
            {
