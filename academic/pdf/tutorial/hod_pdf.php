  ...
  $this->Ln(7);
   ...
   for($i=0;$i<count($header);$i++)
   {
    ...
    if($semister == "Semester II")
            {
                $cat = 5;
                $sqli = "select distinct e.CourseCode, e.assessment_status  from examdate e, submitresult s where  e.CourseCode = s.courseCode and s.dept = $deptid and e.Ayear = $year and e.assessment_status = $cat and e.programme = '$program'";

            }
            else
            {
            
                $sqli = "select distinct e.CourseCode, e.assessment_status  from examdate e, submitresult s where  e.CourseCode = s.courseCode and s.dept = $deptid and e.Ayear = $year  and e.programme = '$program'";
...
$sql2 = "select distinct er.RegNo from examregister er, examdate e where er.CourseCode = e.CourseCode and e.programme = '$program' and er.Ayear = $year and e.Semister  = '$semister' and er.RegNo like '%/%/%' order by er.RegNo asc ";
//die($sql2);
...
if($semister == "Semester II")
            {
                $cat = 5;
                $sql3 = "select distinct e.CourseCode, e.assessment_status  from examdate e, submitresult s where  e.CourseCode = s.courseCode and s.dept = $deptid and e.Ayear = $year and e.assessment_status = $cat and e.programme = '$program'";

            }
            else
            {
            
                $sql3 = "select distinct e.CourseCode, e.assessment_status  from examdate e, submitresult s where  e.CourseCode = s.courseCode and s.dept = $deptid and e.Ayear = $year  and e.programme = '$program'";
