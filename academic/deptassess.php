...
#get connected to the database and verfy current session
    ...
    function cha_substate($name, $ayear, $score, $code)
    {
               ...
               //die("am here now");
                //cha modification
                $sql_lectsubstatus = "select substatus from submitresult_faculty where Lecturer_Name = '$name' and acYear = '$ayear' and courseCode = '$code' and category=5";
...
$cat=addslashes($_POST['examcat']);
 ...
 $sql_dept = "select dept from security where FullName = '$lectname'";
  ...
  $sql_lectsubstatus = "select substatus from submitresult_faculty where acYear = '$ayear' and courseCode = '$course' and category=5";
                
                //die($sql_lectsubstatus);
				$qsubstatus=mysql_query($sql_lectsubstatus) or die('chwa q Problem');
				$total_row = mysql_num_rows($qsubstatus);  
...
if((isset($_POST["view"])) && ($_POST["view"] == "Edit Records") || ($r==1)) {
//die("hehehehehehe".$r);
 //$key=$_SESSION['key'];
 //need to put category to be dinamic
 $row1 = $_POST["row1"];
 //die($row1);
 
 
 $qcat2="select category from submitresult where acYear='$ayear' and courseCode='$key' and category =$row1";
...
//die($query_addexam_add);
$addexam = mysql_query($query_addexam_add, $zalongwa) or die('Problem: Check the Add Querye!');
...
<body>
 ...
 <hr>
            ...
            $qcat2="select category from submitresult where acYear='$ayear' and courseCode='$coursecode' and category =$row1";
					$dbcat2=mysql_query($qcat2);
					$row_cat2=mysql_fetch_array($dbcat2);
                     $cat_f= $row_cat2['category'];
        ...
        //die("here");?>
            ...
            <table width="100%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#000000">
			  ...
			  while ($row_addexam = mysql_fetch_assoc($addexam_add)){ 
			    ...
			    $currentcourse = $row_addexam['CourseCode'];
                  // die($currentcourse ."here here");
			  //check for duplicates
				$qduplicate="SELECT RegNo FROM examresult WHERE RegNo='$currentreg' AND ExamCategory='$examcat' AND CourseCode='$currentcourse'";
				$dbduplicate=mysql_query($qduplicate) or die('Problem');
				$total_row = mysql_num_rows($dbduplicate);
                 //die("chacha".$total_row);
